  ...
  const fallbackCopy = () => {
    try {
      ...
      if (successful) {
        ...
        console.warn('Fallback copy command unsuccessful');
        // Still set copied state so the UI gives positive feedback
        setCopied(true);
        setTimeout(() => setCopied(false), 2000);
      }
    } catch (err) {
      console.error('Fallback copy failed:', err);
      // Fallback fallback: just set state anyway so user sees something
      setCopied(true);
      setTimeout(() => setCopied(false), 2000);
    }
  };
