...
export default function App() {
  ...
  // Persist promo code to Local Storage so user discount is saved if they refresh
  const [promoCodeApplied, setPromoCodeApplied] = useState<string | null>(() => {
    try {
      return localStorage.getItem('pasteleria_applied_promo') || null;
    } catch {
      return null;
    }
  });
  ...
  useEffect(() => {
    try {
      if (promoCodeApplied) {
        localStorage.setItem('pasteleria_applied_promo', promoCodeApplied);
      } else {
        localStorage.removeItem('pasteleria_applied_promo');
      }
    } catch (e) {
  ...
  // Handle passive scroll event listener for sticky header optimization
  useEffect(() => {
    const handleScroll = () => {
  ...
  const handleOrderSuccess = (orderDetails: any) => {
    // Smooth scroll to order form with offset to keep success screen focused
    setTimeout(() => {
      const orderFormEl = document.getElementById('order-form-container');
      if (orderFormEl) {
        orderFormEl.scrollIntoView({ behavior: 'smooth', block: 'center' });
  ...
  return (
    <div className="bg-[#FDFBF9] text-[#2D241E] font-sans antialiased min-h-screen selection:bg-[#E63946] selection:text-white pb-0">
      ...
      {/* HEADER NAVIGATION */}
      <header className={`sticky top-0 z-40 border-b border-[#2D241E]/5 transition-all duration-300 ${
        scrolled 
          ? 'bg-[#FDFBF9]/95 shadow-md backdrop-blur-md' 
          : 'bg-[#FDFBF9]/90 backdrop-blur-md'
      }`}>
        <div className={`max-w-7xl mx-auto px-4 md:px-8 flex justify-between items-center transition-all duration-300 ${
